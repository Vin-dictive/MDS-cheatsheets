...
\begin{lstlisting}[language=R]
round(anova(poisson_model_2, poisson_model, test = "Chi"), 4)
\end{lstlisting}
\smalltext{
$
H_{0} : \text{The simpler model (poisson\_model) is sufficient.} \\
H_{a} : \text{The more complex model (poisson\_model\_2) fits significantly better.}
$
...
\smalltext{
    ...
    $\text{logit}(p_i) = \log \left( \frac{p_i}{1 - p_i} \right) = \beta_0 + \beta_1 X_{i,\text{dance}} + \beta_2 X_{i,\text{val}}$
    }
...
\end{lstlisting}
\smalltext{
$P(Y_i \leq 4 \mid X_{i,1}) = \frac{\exp(\beta_0^{(4)} - \beta_1 X_{i,1})}{1 + \exp(\beta_0^{(4)} - \beta_1 X_{i,1})}$
The categories 1,2,3 in the i-th response $Y_i$ implicate an ordinal scale.
Hence, our Ordinal Logistic regression model will indicate how each one of the 2 regressors that affects the cumulative logarithm of the odds in the ordinal response.
we would need 2 link functions, 2 intercepts, and 2 coefficients.
}
\begin{lstlisting}[language=R]
...
\smalltext{
    ...
    n rows in our training set will not be independent anymore.
}
\subsection{OLS Regression with Varying Intercept}
\smalltext{
We will do this with the lm() function by adding - 1 on the right-hand side of the argument formula. This - 1 will allow the baseline firm to have its intercept (i.e., replacing the usual (Intercept) in column estimate with this specific baseline company’s intercept). In this case, General Motors is the baseline company (as it appears on the left-had side of the levels() output).Note that (1 | firm) allows the model to have a random intercept by firm.
Note that (market\_value + capital | firm) allows the model to have a random intercept and slopes by firm.\textbf{Restricted maximum likelihood} is an estimation approach for Mixed-effects regression. 1. Estimate the variance components. 2.Estimate the fixed-effects given those variance estimates
}
\begin{lstlisting}[language=R]
...
\smalltext{
  $\text{investment}_{i,j} = \beta_{0,j} + \beta_1 \text{marketValue}_{i,j} + \beta_2 \text{capital}_{i,j} + \varepsilon_{i,j}$
}
\begin{lstlisting}[language=R]
...
\smalltext{
    ...
    However, this costs us one extra degree of freedom per firm except for the baseline. Lose 10 degrees of freedom (DF in the anova()).
}
...
\smalltext{
  \textbf{Inference:}
    Each regression coefficient is associated with a \texttt{firm}. For example, \texttt{firmUS Steel:capital} $= 0.02$ means that the variable \texttt{capital} has a slope of $0.02 + 0.37 = 0.39$ for \texttt{US Steel}.
}
\begin{lstlisting}[language=R]
...
\end{lstlisting}
\subsection{Generic Covariance Matrix $D$}
\smalltext{This is a standard form in linear Mixed-effects modelling.}
