...
\textbf{Top-k:} Keeps top $k$ words, renormalizes, and samples;adds diversity. \\
...
\smalltext{
\textbf{Markov Limitations:} Struggle with long-distance dependencies and suffer from sparsity/memory growth as $n$ increases. \\
\textbf{RNN Advantage:} Use \textbf{hidden units} to retain continuous, high-dimensional "memory" of the entire past sequence. \\
\textbf{Graphical Model:} Each state $h_t$ is a function of the current input $x_t$ and the previous state $h_{t-1}$.
}

\subsection{Parameter Sharing}
\smalltext{
RNNs use \textbf{three weight matrices} shared across all time steps:
\begin{itemize}
\item \textbf{$W$:} Input to hidden state ($x_t \to h_t$).
\item \textbf{$V$:} Hidden state to output ($h_t \to y_t$).
\item \textbf{$U$:} Hidden to hidden state ($h_{t-1} \to h_t$).
\end{itemize}
Sharing parameters allows the model to learn patterns regardless of their position in the sequence.
}

\section{Forward Pass \& Math}
\subsection{Calculations}
\smalltext{
\textbf{1. New Hidden State ($h_t$):} Typically uses \code{tanh} activation.
$ h_t = \tanh(U h_{t-1} + W x_t + b_1) $
\textbf{2. Output ($y_t$):} Typically uses \code{softmax} for classification.
$ y_t = \text{softmax}(V h_t + b_2) $
Forward pass iterates through the sequence length $T$.
}

\begin{lstlisting}[language=python]
import torch.nn as nn
# input_size=20, hidden_size=10, layers=1
rnn = nn.RNN(20, 10, 1)
output, hn = rnn(inp, h0)
\end{lstlisting}

\section{Training \& Architectures}
\subsection{BPTT}
\smalltext{
\textbf{Backpropagation Through Time (BPTT):} A generalized backprop for sequential models.
\begin{itemize}
\item Error at $t$ depends on current $y_t$ and all subsequent states $h_{t+1 \dots T}$.
\item \textbf{Truncated BPTT:} Updates using smaller chunks to save cost.
\item \textbf{Issue:} Standard RNNs suffer from \textbf{vanishing gradients} on long dependencies.
\end{itemize}
}

\subsection{Architectures \& Tasks}
\smalltext{
\begin{itemize}
\item \textbf{Many-to-One:} Sequence classification (e.g., Sentiment analysis). Only final $h_n$ is used.
\item \textbf{Many-to-Many:} Sequence labeling (e.g., POS tagging, NER). Output at every step.
\item \textbf{One-to-Many:} Text/Music generation. Sample $y_t$ and use as $x_{t+1}$.
\end{itemize}
}

\subsection{Stacked \& Bidirectional}
\smalltext{
\textbf{Stacked RNNs:} Output of one RNN layer becomes the input for the next; learns hierarchy/abstraction. \\
\textbf{Bidirectional RNNs:} Combines forward RNN ($h_t^f$) and backward RNN ($h_t^b$) to capture context from both sides. Essential for tasks like POS tagging where future words clarify the current word.
}

\smalltext{
\textbf{Many-to-One:} Sentiment analysis (Compressed sequence) \\
\textbf{Many-to-Many:} POS Tagging (Local + Previous context) \\
\textbf{Bidirectional:} Translation (Left + Right context) \\
\textbf{Stacked:} Complex NLP (Higher abstraction layers)
}
