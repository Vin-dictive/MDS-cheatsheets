...
\subsection*{\colorbox{yellow}{Discrete Distribution Families}}
...
\subsection*{\colorbox{yellow}{Continuous Distribution Families}}
...
P(X > x) = \frac{b - x}{b - a}, \quad a \le x \le b 
\]
...
\subsection*{\colorbox{yellow}{Continuous RV (numerical integration)}}

\texttt{$f_X(x)$ can take on values larger than 1} \\
\texttt{$f_X(x)$ cannot give probability, area under the graph does} \\
\texttt{the density of $f_X(x)$ cannot take negative values even if total area is 1 because probability cannot be negative} \\
\texttt{the support of $f_X(x)$ must be finite} \\
\texttt{f <- function(x) 2*x} \\
\texttt{EV <- integrate(function(x) x*f(x), 0,1)\$value} \\
\subsection*{\colorbox{yellow}{Define the PDF function}}
\texttt{f <- function(x) { 2*x }} \\
\texttt{prob <- integrate(f, lower = 0.5, upper = 0.75)\$value \# Integrate over [0.5, 0.75]}
\subsection*{\colorbox{yellow}{Conditional Distributions}}
...
\subsection*{\colorbox{yellow}{Random Sample}}
...
\subsection*{\colorbox{yellow}{MLE}}
\texttt{Way to find estimators. Mean - univariate, linear regression - multivariate}
\subsection*{\colorbox{yellow}{Steps for MLE}}
...
\texttt{3.Find sample and parameters for the distribution to find the most likely estimates} \\
\texttt{4.To get estimates, we use the likelihood function of our observed random sample.} \\
\subsection*{\colorbox{yellow}{R: Generating Sample and Likelihood}}
...
random outcomes, they can generate only 
...
resulting in approx. values that improve 
as the sample size incre (frequentist paradigm)
The Law of Large of no. states that, as we 
increase our sample size n, our empirical mean 
converges to the true mean we want to estimate.
\end{verbatim}
\subsection*{\colorbox{yellow}{Skewness}}
\begin{center}
