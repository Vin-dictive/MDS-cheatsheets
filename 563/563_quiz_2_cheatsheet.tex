...
\end{lstlisting}
\textbf{Euclidean Distance Similarity:|}
\smalltext{
\textbf{Rank 1 (Most Similar):} Item 1. The distance between the tip of the "Query" vector and "item 1" is the smallest.
\textbf{Rank 2: Item 3.} The distance to the tip of "item 3" is significantly larger than item 1.
\textbf{Rank 3 (Least Similar):} Item 2. Despite having a similar angle to item 3, its extreme magnitude places its endpoint furthest from the query.
}
\textbf{Dot Product Similarity:|}
\smalltext{
\textbf{Rank 1 (Most Similar): Item 2.} Although the angle $\theta$ is larger than item 1, the massive magnitude $\|B\|$ of item 2 dominates the product.
\textbf{Rank 2: Item 3.}It has a similar angle to item 2 but a smaller magnitude, resulting in a lower product.
\textbf{Rank 3 (Least Similar): Item 1.} Despite having the smallest angle $\theta$ (highest $\cos(\theta)$), its negligible magnitude results in the smallest dot product.
}
\textbf{Cosine Similarity:|}
\smalltext{
\textbf{Rank 1 (Most Similar): Item 1.} The angular displacement between "Query" and "item 1" is the smallest, meaning $\cos(\theta)$ is closest to 1.
\textbf{Rank 2: Item 2 / Item 3 (Tie).}Visually, item 2 and item 3 appear to lie on the same ray or have very similar angular offsets from the query. If item 3 is slightly closer in angle than item 2, it would take Rank 2.
\textbf{Rank 3 (Least Similar): Item 2 / Item 3.} (The one with the larger angular gap).
}

\subsection{Vector Space Models}
\smalltext{
Map words to dense vectors where geometric distance $\approx$ semantic similarity.
\begin{itemize}
    \item \textbf{One-hot:} $|V|$ dimensions. Sparse. No inherent notion of relationship between words and the dot product between similar and non-similar words is zero.
    \item \textbf{Term-Term Co-occurrence:} go through a corpus of text, keeping a count of all of the words that appear in context of each word (within a window). The similarity is calculated using dot products between word vectors.
    \item \textbf{LSA} count-based/matrix factorization. LSA can generate dense word embeddings. LSA can generate dense word embeddings.
    \item \textbf{Word2Vec}Continuous bag of words (CBOW), Skip-gram optimized by Hierarchical softmax and Negative sampling.
\end{itemize}
}
...
\smalltext{
    ...
    \textbf{word2vec} trained on several corpora using the word2vec algorithm. \textbf{fastText}
...
LSA TruncatedSVD for matrix factorization using SVD (without centering the data).  \textbf{Advantage: }
...
\subsection{Taxonomy of Recommenders}
\smalltext{
\begin{itemize}
...
\subsection{The Filter Bubble \& Ethics}
\smalltext{
...
\section{Problem Formulation \& Baselines}
...
\smalltext{
Approximates $Y \approx ZW^T$, where $Z \in \mathbb{R}^{N \times k}$ and $W \in \mathbb{R}^{M \times k}$. 
\begin{itemize}
    \item \textbf{$z_i$ (User Embedding):} Latent representation of user $i$.
    \item \textbf{$w_j$ (Item Embedding):} Latent representation of item $j$.
\end{itemize}
...
# Define scale and load data
reader = Reader(rating_scale=(1, 5))
data = Dataset.load_from_df(df, reader)
# Initialize SVD with k latent factors
...
\section{Ethics and Consequences}
\smalltext{
