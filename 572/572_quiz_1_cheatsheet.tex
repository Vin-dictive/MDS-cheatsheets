...
\begin{itemize}
    ...
    \item Use an “adaptive” learning rate ADAM optimizer- take big steps when far from the minimum of loss function, and smaller steps when we close to it;
...
\smalltext{
1.Pytorch Training structure.
2.Get a mini-batch from the DataLoader
3.Clear old gradients
4.Forward pass: compute logits (raw scores)
5.Compute the loss
...
\smalltext{
Now to train this, we would need to have a loss function, and try to minimize it, right? Rules of thumb regarding the training:
\begin{itemize}
    \item \textbf{Predict a real number}: 1 output (usually the case, but not necessarily) and we use MSE for loss.
    \item \textbf{Binary classification}: 1 logit and we use BCEWithLogitsLoss for loss.
    \item \textbf{Multi-class (k classes)}: k logits + CrossEntropyLoss (PyTorch expects raw logits).
\end{itemize}
\textbf{Other Common Loss Functions:}
\begin{itemize}
    \item \textbf{Mean Absolute Error (L1 Loss)}: $|y - \hat{y}|$. Less sensitive to outliers than MSE.
    \item \textbf{Huber Loss}: A combination of MSE and MAE. It's quadratic for small errors and linear for large errors.
\end{itemize}
}
\end{multicols}
\end{document}