...
\includegraphics[width=0.3\linewidth, height=1cm]{forecasting.png}
\includegraphics[width=0.3\linewidth, height=1cm]{holt.png}
\includegraphics[width=0.3\linewidth, height=1cm]{ETS.png}
...
\begin{lstlisting}[language=python]
model = ETSModel(df["Production"], error="add", trend="add", damped_trend=True, seasonal="mul", seasonal_periods=12).fit()
predictions = model.get_prediction(start=df.index[-1] + pd.DateOffset(months=1), end="2020").summary_frame()
q = 0.975  # 95% prediction intervals
simulations = model.simulate(anchor="end", nsimulations=348, repetitions=100, random_errors="bootstrap")
fig = plot_acf(model.resid, lags=60, title=None, bartlett_confint=False)
p1 = model.test_serial_correlation('ljungbox', lags=24)[0, 1, -1]
p2 = model.test_normality('jarquebera')[0, 1]
if p1 < 0.05: print("Residuals significantly different from white noise. That's bad...")
if p2 < sig_level: print("Residuals significantly different from normal distribution. That's bad...")
\end{lstlisting}
...
\smalltext{
  ...
  \textbf{Hypothesis tests} Augmented Dickey Fuller (“ADF”) test or Kwiatkowski-Phillips-Schmidt-Shin (“KPSS”) test: Null hypo $H_0$: The time series is non-stationary, small p-value indicates stationary.  
}
...
\begin{lstlisting}[language=python]
  result = adfuller(noise) #stationary check
...
\includegraphics[width=0.5\linewidth, height=1cm]{ARIMA.png}
\includegraphics[width=0.5\linewidth, height=1cm]{SARIMA.png}
