...
Used for nominal (unordered) categorical features. It creates a new binary column for each category.
    \textcolor{red}{`handle\_unknown="ignore"`:} If an unseen category appears during testing/validation, it's ignored, and the resulting one-hot vector will have zeros for all category columns.
    \textcolor{red}{`sparse\_output=False`:} Returns a dense NumPy array instead of a sparse matrix.
    \textcolor{red}{Binary Features (`drop="if\_binary"`):} For a feature with only two categories (e.g., "yes"/"no"), this argument creates only one resulting column (e.g., $1$ for "yes" and $0$ for "no"), avoiding redundancy.
}
...
Used for ordinal (ordered) categorical features (e.g., "Good", "Average", "Poor"). It maps each category to an integer value.
    \textcolor{red}{Difference from OHE:} Ordinal encoding uses a single column of integers, assuming an order. OHE uses multiple binary columns and assumes no order.
    \textcolor{red}{rdering:} Manually ordering categories is important to correctly reflect the underlying relationship.
    \textcolor{red}{Unknown Categories:} Without careful configuration, an unknown category during testing/validation could lead to an error or an arbitrary value assignment, impacting model performance.
    \textcolor{red}{Multiple Ordinal Columns:} Pass a \textbf{list of lists} to the `categories` parameter of `OrdinalEncoder`, where each inner list contains the ordered categories for one column.
}
...
\textcolor{red}{Popular Representations:} Bag-of-Words (BOW), TF-IDF, and Embedding representations.
}
...
\begin{itemize}
    ...
    \item \textbf{Preprocessing:} By default, performs \textbf{lowercasing} and gets rid of \textbf{punctuation}.
...
\begin{itemize}
    \item \textbf{Purpose:} Easier interpretation, maintaining privacy (e.g., children-5, teenager-15, young adults-25), or capturing non-linear relationships in linear models for easier interpretation or to maintain privacy, or to capture non-linear relationships.
...
\includegraphics[width=0.5\linewidth, height=1.5cm]{opti.png}
...
param_grid = {
    "svc__gamma": loguniform(1e-5, 1e3), # Exponential ranges are common
...
\textbf{Binary Feature Vectors:} This classifier assumes that all feature vectors are binary (0s and 1s). For a document, a feature $x_i$ is 1 if the word is present and 0 if it is absent. \\
\textbf{Naive Assumption:} it assumes that the features are \textbf{conditionally independent} given the class $C_k$. \\
\textbf{Smoothing (Additive/Laplace Smoothing):} To prevent zero probabilities when a feature does not occur with a certain class, Laplace smoothing is applied:,The default smoothing parameter $\alpha$ is typically 1. High $\alpha$ means overfitting (means we are adding large counts to everything and so we are diluting the data), low means underfitting. \\
\textcolor{red}{\textbf{Gaussian Naive Bayes}}This variant is used when features are continuous. \\ 
\textbf{Feature Vectors:} Assumed to be continuous and normally distributed. If not make it, using powerTranformer() (e.g., height, temperature) \\
\textbf{Conditional Probability (Likelihood):} The likelihood $P(x_i | C_k)$ is calculated using the Probability Density Function (PDF) of the Normal distribution \\
\textbf{Training:} The model simply needs to compute the $\mu_{ik}$ and $\sigma_{ik}^2$ for every feature $i$ and every class $C_k$ from the training data. There is no concept of "smoothing" as used in discrete models. \\
}
\begin{lstlisting}[language=Python]
...
where $\mathbf{w}$ is the vector of coefficients (weights) and $b$ is the bias (intercept).
    \textcolor{red}{Linear Regression (Regression):} Predicts a \textbf{continuous} output $\hat{y} \in \mathbb{R}$. It optimizes the loss function (e.g., Mean Squared Error) to minimize the distance between $\hat{y}$ and the true output $y$.
    \textcolor{red}{Logistic Regression (Classification):} Predicts a \textbf{probability score} for binary classification. The linear output is passed through the sigmoid function to map it to $[0, 1]$.
    \textcolor{red}{Linear Support Vector Machine (Linear SVM):} A classification model that finds the hyperplane with the largest margin separating the classes. It is generally robust and efficient.
\subsection*{Model Parameters and Optimization}
\textcolor{red}{Hyperparameter:} A parameter whose value is set \textbf{prior} to the training process (e.g., $\alpha$ in Lasso/Ridge regularization, C in SVMs). It controls the model's complexity and learning rate.
\textcolor{red}{Interpretation of Coefficients ($w_i$):} The magnitude and sign of a coefficient $w_i$ indicate the strength and direction of the relationship between the feature $x_i$ and the prediction $\hat{y}$, assuming all other features remain constant.
\subsection*{Probability Scores and Functions}
\textcolor{red}{Predicting Probability Scores:} For classification, linear models often output a score that can be interpreted as the probability of belonging to a certain class (e.g., in Logistic Regression).
    \textcolor{red}{The Sigmoid Function ($\sigma$):} Used in binary classification (like Logistic Regression) to map the linear output (logit) to a probability between 0 and 1.
    \subsection*{Sigmoid vs. Softmax:}
    \textcolor{red}{Sigmoid:} Used for \textbf{binary classification} or independent multi-label classification.
     \textcolor{red}{Softmax:} Used for \textbf{multi-class classification} (one class mutually exclusive from others). It converts a vector of scores into a probability distribution that sums to 1.
\subsection*{Case Analysis}
For a classification task predicting $P(\text{Class A})$:
\textcolor{red}{Most Confident Cases:} Predictions closest to 0 or 1 (e.g., 0.01 or 0.99). These are cases where the model is highly certain about its classification.
\textcolor{red}{Least Confident Cases:} Predictions closest to the decision boundary (e.g., 0.49 or 0.51). These are cases where the model is uncertain and the data point is near the separating hyperplane.
\textcolor{red}{Over Confident Cases:} High probability predictions (near 0 or 1) that turn out to be \textbf{wrong}. This often indicates poor calibration or overfitting.

\subsection*{Multi-Class Classification}
Handled either by extending the linear function to output a score for each class (e.g., using the \textbf{Softmax} function) or by decomposing the problem into multiple binary problems (e.g., One-vs-Rest).

\subsection{Strengths of Linear Models:}
\textcolor{red}{Interpretability:} Coefficients provide clear insight into feature importance.
 \textcolor{red}{Efficiency:} Fast to train and predict, and scale well to large datasets.
\textcolor{red}{Simplicity:} Less prone to overfitting on low-dimensional data.
\subsection{Limitations of Linear Models:}
\textcolor{red}{Limited Expressiveness:} Cannot capture complex non-linear relationships without manual feature engineering (e.g., adding polynomial features).
  \textcolor{red}{Sensitivity to Outliers:} Especially for Linear Regression, they can be easily skewed by extreme values.
 \textcolor{red}{Independence Assumption:} Performance suffers if features are highly dependent on each other.
\end{multicols}
\end{document}